...
final_output_events=int((2e10)) #Number of cells to be created by the final terminally differentiated level
gamma=2.05 #Value of gamma for the non mitotic pool
gamma_progenitor=3.2 #Value of gamma for the mitotic pool
...
#            rates_matrix_par[i,0]=rates_matrix_par[i+1,0]/gamma_cfc
    ...
    def set_leaking():
        for i in range(0,number_levels_par-1):
            rates_matrix_par[i,7]=2e7*math.exp(-lambda_parameter*(number_levels_par-i))
...
def number_of_events(initial_rates_par,number_levels,array,bm_cellu_par):
    ...
    global rho
    delta_t=np.array([1/((np.amax(initial_rates_par[np.where(blood>0.),:2]))*10)])
    if array=='bone_marrow':
        current_cellularity=np.sum(bone_marrow)
        leaking_function=math.exp(leak_par*(current_cellularity-bm_cellu_par))
#        print(leaking_function)
        local_rates=np.multiply(bone_marrow.reshape(bone_marrow.shape[0],1),initial_rates_par[0:np.shape(bone_marrow)[0],:])
        local_rates=np.multiply(local_rates,delta_t)
        local_rates[:,4]=local_rates[:,4]*(leaking_function)
        local_rates[:,:2]=local_rates[:,:2]
...
#        print(local_rates)
        ...
        local_rates=np.multiply(blood.reshape(blood.shape[0],1),initial_rates_par[0:np.shape(blood)[0],:])
        local_rates=np.multiply(local_rates,delta_t)
...
def simulation_step(initial_rates):
    ...
    global rho
    global delta_t
    mod_original_cellularity=3*bm_original_cellularity
...
dummy=0
while dummy<num_sim:
    fitness_s=0.14 #fitness of the mutant cells
    files_name='test_'+str(dummy)
...
#    bone_marrow[number_levels:2*number_levels]=homeostasis/0.85
    fitness_s=0.05 #fitness of the mutant cells
    add_block_rates(number_levels,np.size(blood))
#    add_block_rates(number_levels,np.size(blood))
#    add_block_rates(number_levels,np.size(blood))
#    add_block_rates(number_levels,np.size(blood))
